...
- ✅ **ROLE-BASED DYNAMIC TEXT & LABELS** - Intelligent UI text that adapts to user role selected during onboarding: (1) **Student Mode**: "Groups" tab becomes "Friends", page header shows "Friends" with description "Connect and study with friends", tab labels show "Friend Groups" and "Study with Friends", (2) **Teacher Mode**: "Groups" tab becomes "Students", page header shows "Students" with description "Communicate and collaborate with your students", tab labels show "My Students" and "Live with Students", (3) **Navigation Integration**: Bottom tab label dynamically updates based on user role, (4) **Consistent Experience**: All headings, descriptions, tab labels, and tooltips update consistently throughout the app, (5) **Persistence**: Role-based text persists across app restarts, logout/login cycles, and syncs with user profile data stored in AsyncStorage, (6) **Seamless UX**: Text changes are instant and require no app restart, providing immediate context-appropriate language. Perfect for dual-purpose app serving both students and teachers.
